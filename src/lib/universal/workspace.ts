...
export const workspaceEditorTargetResolvers = ["vscode", "vscode-wsl", "vscode-ssh-remote", "vscode-windows", "vscode-linux", "vscode-mac"] as const;
    ...
    switch (type) {
        ...
        case "vscode-ssh-remote":
            return vscodeSshRemoteEditorResolver(
                options?.vscodeSsshRemoteHostname?.() ?? `${type}'s vscodeSsshRemoteHostname() supplier not supplied`,
            );
        case "vscode-windows":
...
export function gitLabResolvers(
    ...
    const routeGitRemoteResolver: RouteGitRemoteResolver<
        GitRemoteAnchor
    > = (fileSysPath, branch, paths) => {
                ...
                if (asset) {
                    ...
                    if (href) {
                        const result: GitRemoteAnchor = {
                            ...
                            href,
                            textContent:
                                `${fileSysPath.slice(fileSysPath.lastIndexOf('/')+1)} on ${remoteServerHumanName}`,
                        };
                        return result;
                    }
                }
