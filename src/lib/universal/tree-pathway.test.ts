...
describe('Path Tree Node Pathway', () => {
  ...
  it('populates path tree pathway from generator', () => {
    ...
    const bcUnit: tp.TreePathwayUnitSupplier<Terminal, Intermediary, BreadcrumbUnit> = (node) => ({ label: node.unit, slug: node.qualifiedPath });

    const prepareBreadcrumbs1 = tp.treePathwaysPreparer(tree, bcUnit, bcUnit, { includeTerminal: bcUnit });
